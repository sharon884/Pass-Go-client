...
const EventDetails = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentImageIndex(
        (prevIndex) => (prevIndex + 1) % event.images.length
      );
  ...
  if (!event)
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-50">
        <div className="animate-pulse text-[#5B3DF5] text-xl font-semibold">
          Loading....
        </div>
      </div>
    );
          ...
          <div className="bg-gray-50 rounded-xl p-6 shadow-sm border border-[#D1D5DB]">
            ...
            <div className="mb-4 p-4 bg-white rounded-lg border border-[#D1D5DB]">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-[#5B3DF5] font-bold">
                  {event?.tickets?.VIP?.price ?? "N/A"}
                </span>
              </div>
              <div className="text-sm text-gray-500">
                Available: {event?.tickets?.VIP?.quantity ?? "N/A"}
              </div>
            </div>
            ...
            <div className="mb-4 p-4 bg-white rounded-lg border border-[#D1D5DB]">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-[#5B3DF5] font-bold">
                  {event?.tickets?.general?.price ?? "N/A"}
                </span>
              </div>
              <div className="text-sm text-gray-500">
            ...
            <button
              ...
              className="w-full bg-[#5B3DF5] hover:bg-[#4930c4] text-white font-medium py-3 rounded-lg transition-colors duration-300 mt-2"
            >
              Get Tickets
            </button>
          </div>
  ...
  <EventDistanceMap eventCoords={event.coordinates} />
)}
