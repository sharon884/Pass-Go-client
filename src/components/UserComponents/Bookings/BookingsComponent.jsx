          ...
          >
            <div className={`border-b ${styles.borderColor}`}>
              <nav className="flex overflow-x-auto">
                {[
                  { key: "all", label: "All Bookings", count: orders.length + tickets.length },
                  { key: "orders", label: "Paid Orders", count: orders.length },
                  { key: "tickets", label: "Free Tickets", count: tickets.length },
                ].map((tab) => (
                  <button
                    key={tab.key}
        ...
        <div className="mb-6">
          ...
          >
            <div className={`border-b ${styles.borderColor}`}>
              <nav className="flex overflow-x-auto">
                {[
                  { key: "all", label: "All Events", count: filteredBookings.length },
                  {
                    key: "upcoming",
                    label: "Upcoming",
        ...
        {filteredBookings.length === 0 ? (
          ...
          >
            ...
            {filter === "all" && (
              <a
                href="/events"
                className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm ${styles.buttonPrimary} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500`}
              >
                Browse Events
              </a>
            )}
                      ...
                      >
                        <td className="px-4 py-4">
                          <div className="flex items-center">
                            ...
                            <div className="ml-4 min-w-0 flex-1">
                              <div className={`text-sm font-medium ${styles.textPrimary} truncate`}>
                              ...
                              <div className="flex items-center space-x-2 mt-1">
                                ...
                                <span
                                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getBookingTypeColor(booking.bookingType)}`}
                                >
                        ...
                        <td className="px-4 py-4">
                          <div className="space-y-1">
                            {booking.category && (
                              <span
                                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getCategoryBadgeColor(booking.category)}`}
                              >
                                {booking.category}
                              </span>
                            )}
                        ...
                        <td className="px-4 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(booking.status, booking.bookingType)}`}
                          >
                            {formatStatus(booking.status)}
                          </span>
                        </td>
                        <td className={`px-4 py-4 whitespace-nowrap text-sm ${styles.textMuted}`}>
                          {formatDate(booking.createdAt || booking.bookedAt)}
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <a href={`/booking/${booking._id}`} className="text-indigo-600 hover:text-indigo-900">
                            View
                          </a>
                        </td>
                      </tr>
            ...
            <div className="xl:hidden space-y-4">
              {filteredBookings.map((booking) => (
                ...
                >
                  {/* Card Header */}
                  <div
                    className={`px-4 py-4 bg-opacity-50 ${currentTheme === "classic" ? "bg-gray-50" : "bg-gray-800"} border-b ${styles.borderColor}`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className={`text-base font-medium ${styles.textPrimary} truncate`}>
                            {booking.eventTitle || "Event not found"}
                          </h3>
                      ...
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(booking.status, booking.bookingType)} ml-2 flex-shrink-0`}
                      >
                        {formatStatus(booking.status)}
                      </span>
                    </div>
                  ...
                  <div className="px-4 py-4 space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 flex-1 min-w-0">
                        ...
                        <span className={`text-xs ${styles.textMuted} truncate`}>{booking.eventCategory}</span>
                      </div>
