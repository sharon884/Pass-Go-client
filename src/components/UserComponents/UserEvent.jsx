...
const UserEvents = () => {
  ...
  const fetchEvents = async (page = 1) => {
    try {
      const data = await fetchApproveEvents(page, 3, eventType, sortBy)
      ...
      console.log("Pagination data:", {
        ...
        totalPages: data.totalPages || Math.ceil((data.total || data.events?.length || 0) / 8),
        totalEvents: data.total || data.events?.length || 0,
        eventsPerPage: 8,
      })
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      {/* Enhanced Pagination - Show for debugging */}
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-12 sm:mt-16">
          {/* Debug info */}
          <div className="text-xs mb-2 sm:mb-0" style={{ color: styles.textSecondary }}>
            Debug: Page {page} of {totalPages} (Total: {events.length} events)
          </div>
