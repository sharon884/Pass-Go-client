...
export const eventValidationSchema = z
  .object({
    ...
    tickets: z.object({
      VIP: z.object({
        ...
        quantity: z.number({ invalid_type_error: "VIP quantity must be a number" }),
      }),
      general: z.object({
        price: z.number({ invalid_type_error: "General price must be a number" }),
        quantity: z.number({ invalid_type_error: "General quantity must be a number" }),
      }),
    }),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.eventType !== "free") {
      ...
      if (
        VIP.price <= 0 || VIP.quantity <= 0 ||
        general.price <= 0 || general.quantity <= 0
      ) {
        ctx.addIssue({
          path: ["tickets", "VIP", "price"],
          code: z.ZodIssueCode.custom,
          message: "Ticket prices and quantities must be greater than 0 for paid events",
        })
      }
    }
  })
